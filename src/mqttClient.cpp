...
void mqttClient::run() {
  ...
  lws_set_log_level(LLL_USER | LLL_ERR | LLL_WARN | LLL_NOTICE | LLL_CLIENT,
                    NULL);
...
int mqttClient::connect(struct lws_context *pcontext) {
  ...
  spdlog::info(
      "connection client_id:[{}],username:[{}],address:[{}],host:[{}],port:[{}]",
...
void mqttClient::publish(std::string &p_topic,
                         const std::shared_ptr<MESSAGE> &p_message) {
  spdlog::info("mqtt publish added to queue, queue size = {}", messages.size());
  MqttMessage_ msg = std::make_pair(p_topic, p_message);
  //msg.first is p_topic msg.second is p_message
  {
...
void mqttClient::onClientWriteAble(struct lws *wsi, struct pss *pss) {
  ...
  case STATE_PUBLISH_QOS0: {
    ...
    spdlog::info("publish topic [{}], len [{}]", pub_param.topic,
                 pub_param.payload_len);
    if (lws_mqtt_client_send_publish(wsi, &pub_param,
                                     elem.second->Union.content,
                                     pub_param.payload_len, 1)) {
...
int mqttClient::callback(struct lws *wsi, enum lws_callback_reasons reason,
...
void mqttClient::addWsiInstance(std::string &componentName, struct lws *wsi) {
  ...
  if (itmap == wsi_map.end()) {
    std::pair<std::string, struct lws *> instance = std::make_pair(
        componentName, wsi);
